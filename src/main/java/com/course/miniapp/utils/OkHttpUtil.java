...
public class OkHttpUtil {
    ...
    public static final OkHttpClient okHttpClient = OkHttpUtil.getOkHttpClient();
    private OkHttpUtil() {
    }
     ...
     * post请求，获取响应结果
     */
    public static String post(OkHttpClient okHttpClient, String url, Map<String, String> body, Headers headers) {
        log.info("okHttpClient post url:{}, body====> {}", url, body);
        // 创建MultipartBody.Builder，并设置类型为form-data
    ...
    public static String post(OkHttpClient okHttpClient, String url, Headers headers) {
        log.info("okHttpClient post url:{}", url);
        // 创建MultipartBody.Builder，并设置类型为form-data
//        MultipartBody.Builder builder = new MultipartBody.Builder().setType(MultipartBody.FORM);
        Request request = new Request.Builder().url(url).headers(headers).post(RequestBody.create(null, new byte[0])).build();
    ...
    public static String post(OkHttpClient okHttpClient, String url, Map<String, String> body) {
        Headers headers = new Headers.Builder().build();
        return post( okHttpClient,  url,  body, headers);
    }

    /**
     * post请求。使用默认的 okHttpClient 和 headers
     */
    public static String post( String url, Map<String, String> bodyJson) {
        //使用默认的 okHttpClient
        OkHttpClient okHttpClient = getOkHttpClient();
        Headers headers = new Headers.Builder().build();
